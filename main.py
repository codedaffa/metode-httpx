...
@app.post("/httpx")
def input_httpx(input: DataRequest):
    try:
...
def scrape(base_url, headers, keyword, page, session):
    ...
    try:
        ...
        print(f"Gagal melakukan koneksi ke halaman {page}")
        return []
...
def data_product(soup_produk, product_link, session, headers):
    try:
        ...
        print(f"Gagal melakukan koneksi ke halaman {product_link}")
        return None


def data_shop(shop_link, session, headers):
    try:
        response = session.get(shop_link, headers=headers)
        response.raise_for_status()
        soup = BeautifulSoup(response.content, "html.parser")
        ...
        print(f"Gagal melakukan koneksi ke halaman {shop_link}")
        return None
